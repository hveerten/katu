...
def theta_to_params(theta):
    ...
    log_R, log_B, log_S_f, Gamma, log_density = theta

    return (10**log_gamma_min, 10**log_gamma_break, 10**log_gamma_max,
            first_slope, second_slope,
            10**log_R, 10**log_B, 10**log_S_f, Gamma, 10**log_density)
...
def params_to_theta(params):
    ...
    return (np.log10(gamma_min), np.log10(gamma_break), np.log10(gamma_max),
            first_slope, second_slope,
            np.log10(R), np.log10(B), np.log10(S_f), Gamma, np.log10(density))
...
for i in range(100):
    ...
    p1_mcmc, p2_mcmc, \
    log_R_mcmc, log_B_mcmc, log_S_f_mcmc, Gamma_mcmc, log_density_mcmc = \
    ...
    print("{:.3f} ± {:.3f}".format(log_B_mcmc[0],    (log_B_mcmc[1]    + log_B_mcmc[2])    / 2), end='\t')
...
samples = sampler.chain[:,:,:].reshape((-1,ndim))
fig = corner.corner(samples, labels=labels, range=limits, truths=real_params)
